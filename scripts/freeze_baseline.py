...
def copy_acceptance_artifacts(
    ...
    *,
    ...
    """Copy compact evaluation sidecars generated beside evaluation_report.json."""
    copied: dict[str, dict[str, object]] = {}
    missing = []
    for name in ACCEPTANCE_ARTIFACT_NAMES:
        src = report_path.parent / name
        if src.is_file():
            copied[name] = copy_optional_artifact(src, dst_dir)
        else:
            missing.append(str(src))
    if required and missing:
        missing_text = "\n  ".join(missing)
        raise FileNotFoundError(
            "Missing acceptance artifacts generated by evaluate_checkpoint.py:\n  "
            + missing_text
        )
    return copied
...
def main() -> None:
    ...
    acceptance_artifacts = copy_acceptance_artifacts(
        report_path,
        out_dir,
        required=not args.skip_report_verify,
    )
    ...
    if not args.skip_report_verify:
        verify_cmd.append("--require_acceptance_artifacts")
