...
| 5 | Minimal feature ablations E0-E3 | Ready to run, E0/E1 smoke-tested | `configs/feature_e0_baseline.yaml` through `configs/feature_e3_path_phase.yaml`; `scripts/run_feature_ablation.py` supports fixed split, seed, XML, and checkpoint protocol; `results\e0_e1_training_smoke.md` verifies E0/E1 training entrypoints with fixed split and reconstructed XML. | Run E0 first as the only control, then E1-E3 under the same protocol. |
